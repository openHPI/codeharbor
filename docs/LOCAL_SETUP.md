...
3. Solid Queue supervisor (optional):

  ```shell
  bundle exec rake solid_queue:start
  ```
  
Solid Queue is currently only used to synchronize task metadata with the "NBP Datenraum".
The synchronization is disabled by default. Unless you enable it, you can skip starting the supervisor.
