...
RSpec.describe ProformaService::ConvertZipToTasks do
  describe '.new' do
    ...
    context 'with all arguments' do
      subject(:convert_zip_to_tasks) { described_class.new(zip_file: zip_file, depth: depth, path: path) }
  ...
  describe '#execute' do
    ...
    let(:zip_file) { fixture_file_upload('files/proforma_import/testfile.zip', 'application/zip') }
    let(:importer) { instance_double('Proforma::Importer', perform: task) }
    ...
    it 'returns an array with a hash for the task' do
      expect(convert_zip_to_tasks).to eql [{path: 'testfile.zip', uuid: 'uuid', task: task}]
    end
    ...
    context 'when zip_file contains multiple zipped tasks' do
      ...
      it 'returns an array with a hash for the task' do
        expect(convert_zip_to_tasks).to eql [
          {path: 'testfile_multi.zip/task_24-short_hello_world.zip', uuid: 'uuid', task: task},
          {path: 'testfile_multi.zip/task_1-haha_world__123456.zip', uuid: 'uuid', task: task},
          {path: 'testfile_multi.zip/task_2-hallo_welt.zip', uuid: 'uuid', task: task}
        ]
      end
    end
  end
