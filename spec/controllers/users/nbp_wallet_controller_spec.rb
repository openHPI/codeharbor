...
RSpec.describe Users::NbpWalletController do
  ...
  describe 'GET #connect' do
    ...
    context 'when connector is down' do
    ...
    context 'when display name is cached' do
      before do
  ...
  describe 'GET #relationship_status' do
    ...
    context 'when connector is down' do
      ...
      it 'redirects to connect page' do
        get_request
        expect(response).to redirect_to(nbp_wallet_connect_users_path)
      end
    end

    context 'when no relationship created yet' do
      before do
        stub_request(:get, "#{connector_api_url}/Relationships?status=Pending")
          .to_return(body: file_fixture('enmeshed/no_relationships_yet.json'))
      end

      it 'returns json with waiting status' do
        get_request
        expect(response.parsed_body['status']).to eq 'waiting'
      end
    end

    context 'when relationship is expired' do
      ...
      it 'tries to reject RelationshipChange' do
        get_request
        expect(reject_request_stub).to have_been_requested
      end
    end

    context 'when relationship is valid' do
      let(:reject_request_stub) { stub_request(:put, "#{connector_api_url}/Relationships/RELoi9IL4adMbj92K8dn/Changes/RCHNFJ9JD2LayPxn79nO/Reject") }
      ...
      it 'returns json with ready status' do
        get_request
        expect(response.parsed_body['status']).to eq 'ready'
      end

      it 'does not reject RelationshipChange' do
        get_request
  ...
  describe '#finalize' do
    ...
    context 'without errors' do
      it 'accepts RelationshipChange' do
        get_request
      ...
      it 'does not create confirmed user' do
        get_request
    ...
    context 'when user cannot be created' do
      ...
      it 'rejects RelationshipChange' do
        get_request
        expect(reject_request_stub).to have_been_requested
      end
    ...
    context 'when RelationshipChange cannot be accepted' do
