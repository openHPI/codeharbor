...
class Task < ApplicationRecord
  ...
  # belongs_to :predecessor, class_name: 'Task', optional: true
  # has_one :successor, class_name: 'Task', foreign_key: 'predecessor_id', inverse_of: :predecessor, dependent: :nullify
  ...
  #     task.exercise_files = duplicate_files_without_testfiles
  #   end
  # end
  ...
  #   root_exercise = Task.unscoped.find(id)
  ...
  #     file.task.tests.map(&:exercise_file).include? file
  #   end.map(&:duplicate)
  # end
  ...
  #   errors.add(:task, 'has more than one primary descriptions') if primary_description_count > 1
  #   errors.add(:task, 'has no primary description') if primary_description_count < 1
  # end
  ...
  #   self.license_id = Task.find(params[:exercise_relation][:origin_id]).license_id if params[:exercise_relation]
